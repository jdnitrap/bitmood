...
//
// Format "CMX8":
//   magic "CMX8" | type u8 | width u32 | channels u8 | sample rate u32 | LSTM cells u8
//   | graph u8 | graph confirm u32 | snn u8 | snn leak f32 | graph max nodes u32 (little-endian)
...
int cmd_decompress(int argc, char** argv, int start) {
  ...
  for (const char* old : {"CMXB", "CMX2", "CMX3", "CMX4", "CMX5", "CMX6", "CMX7"})
  ...
  if (in.size() < 4 || std::memcmp(in.data(), "CMX8", 4) != 0) throw std::runtime_error(path + ": not a CMX8 file");
